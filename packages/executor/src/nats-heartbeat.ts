...
import type { FleetHeartbeat, NodeId } from '@boardroom/shared';
...
function getSystemMetrics(): {
  ...
  try {
    if (IS_WINDOWS) {
      ...
      const loadAvg = parseFloat(
        execSync('cat /proc/loadavg', { encoding: 'utf-8' }).split(' ')[0],
      );
  ...
  let gpuMemPercent: number | undefined;
...
export function startNatsHeartbeat(
  nc: NatsConnection,
  nodeId: NodeId,
  ...
  const send = () => {
    ...
    nc.publish(
      FLEET_SUBJECTS.heartbeat(nodeId),
      new TextEncoder().encode(JSON.stringify(hb)),
    );
  };
